...
// Aufgabe strToUpper Anfang 
    public static String strToUpper(String original) {
   	 ...
   	 while (i < strLen(original)) { //Der eingegebene String wird bis zum Ende durchlaufen
   		 ...
   		 // oder den Umlauten ä,ö und ü entspricht, wird der Char, der an der selben Stelle 
   		 // im großBuchstaben Array steht, in den umwandlungsHilfe String geschrieben.
   		 ...
   		 // Falls das Char nicht die oberen Kriterien erfüllt, 
   		 // wird es unverändert in den umwandlungsHilfe String geschrieben, 
   		 // da es sich dann entweder um einen Großbuchstaben oder um ein
...
// Aufgabe strToUpper Ende
    
// Aufgabe strSplit Anfang
    ...
    public static String[] strSplit(String original, char delimiter) {
   	 int arrayiterator = 0;
   	 int arrayGröße = 1; // Das Array braucht mindestens ein Feld das einen 
        // leeren String aufnehmen kann.
   	 int originalStringIterator = 0;

   	 // Hier werden vor dem Auftrennen in einzelne Strings die vorhandenen "delimiter"
 // gezählt.
   	 ...
   	 strSplit[0] = new String(); 	//Das erste String Element wird initialisiert um die 
// Ausgabe von "null" zu verhindern.

   	 while (originalStringIterator < strLen(original)) {
   		 
   		 // Falls das aktuell geprüfte Char ein "delimiter" ist, 
 // wird das nächste Arrayfeld ausgewählt.
   		 if (strCharAt(original, originalStringIterator) == delimiter) {
   			 arrayiterator++;
   			 strSplit[arrayiterator] = new String(); // Vor dem Nutzen des neuen Feldes 
  // wird dieses initialisiert.
   		 }
   		 
   		 // Wenn der aktuelle char kein "delimiter" ist, 
 // wird er dem String in dem Ausgewählten Arrayfeld hinzugefügt.
   		 else {
...
// Aufgabe strSplit Ende
    
// Aufgabe strScan Anfang
    // Die Funktion durchsucht den String "original" nach einem Teil-String "needle" und gibt dessen
    // Position innerhalb von "original" zurück. Wenn keiner gefunden wird, wird -1 zurückgegeben.
    static int strScan(String original, String needle) {

   	 // Prüfung ob einer der Strings leer ist, in dem Fall ist keine Übereinstimmung 
 // möglich und es wird -1 ausgegeben.
 ...
 // die in dem orginal String,
   			 ...
   			 if (needleZähler > 0) { // Dies ist der Fall wenn ein (oder mehrere) 
        // übereinstimmende Chars gefunden wurden.
   				 
   				 // Zurücksetzen des needleZählers, falls bereits ein oder mehrere 
 // identische Chars gefunden wurden
   				 // und der darauffolgende dann nichtmehr dem needle entspricht. 
 // In diesem Fall wird bei dem nächsten
   				 // Schleifendurchlauf der originalZähler um 1 erhöht und ab dieser 
 // Stelle erneut geprüft.
